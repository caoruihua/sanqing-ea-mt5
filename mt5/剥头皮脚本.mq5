...
//| 版本历史：                                                        |
//| v1.6 (2026-04-16)                                                |
//|   - 新增结构性反转危险区过滤：上涨末端禁追多、下跌末端禁追空      |
//|   - 新增突破释放：有效突破危险区后恢复原顺势开仓逻辑              |
//|   - 新增长影线、吞没/压入、短线结构突破/跌破等反转证据判断        |
//|   - 强趋势直接入场阈值从0.4%调整为0.2%                            |
//|   - 新增到达结构反转观察区日志，便于排查过滤触发原因              |
//|                                                                   |
//| v1.5 (2026-04-16)                                                |
//|   - 新增自动平仓时间：每天8:55自动平仓所有持仓                    |
//|   - 新增禁止交易时间段：8:55-9:05、22:25-22:35禁止交易            |
//|                                                                   |
//| v1.4 (2026-04-15)                                                |
...
enum ENUM_BREAKOUT_CONFIRM_MODE
{
   BREAKOUT_BY_TICK = 0,       // 当前tick突破即解除禁入
   BREAKOUT_BY_CLOSED_BAR = 1, // 已收盘K线突破才解除禁入
   BREAKOUT_BY_EITHER = 2      // tick或已收盘K线任一突破即解除禁入
};
...
input double StrongTrendRatio    = 0.002;      // 强趋势比例，价格偏离EMA超过此比例直接入场，不等回调
input int    SustainedTrendBars  = 5;          // 持续趋势入场：连续N根K线在EMA同侧
input double SustainedTrendMinDeviation = 0.001; // 持续趋势入场：最小偏离比例（默认0.1%）

input group "===== 结构性反转过滤参数 ====="
input bool   EnableStructuralReversalFilter = true;   // 是否启用结构性反转危险区过滤
input int    ReversalLookbackBars = 24;                // 危险区回看K线数，M5下24根约2小时
input int    ReversalAtrPeriod = 14;                   // 冲刺阈值ATR周期
input double ReversalSprintAtrMultiplier = 2.8;        // 冲刺行情阈值：ATR倍数
input double ReversalHighZoneRatio = 0.80;             // 高位危险区比例，>=0.80视为接近高位
input double ReversalLowZoneRatio = 0.20;              // 低位危险区比例，<=0.20视为接近低位
input double ReversalShadowBodyRatio = 1.5;            // 长影线/实体最小比例
input double ReversalShadowOppositeRatio = 3.0;        // 主影线相对反向影线最小倍数
input bool   EnableReversalEngulfingCheck = true;      // 是否启用吞没/压入式反转K线检查
input int    ReversalStructureBreakBars = 5;           // 短线结构突破/跌破回看K线数
input double ReversalBreakoutBufferUsd = 0.50;         // 突破危险区缓冲，突破后恢复顺势交易
input ENUM_BREAKOUT_CONFIRM_MODE ReversalBreakoutConfirmMode = BREAKOUT_BY_EITHER; // 突破确认方式
...
input group "===== 时间控制参数 ====="
input bool   EnableTimeControl    = true;       // 是否启用时间控制（自动平仓和禁止交易时段）
input int    AutoCloseHour        = 8;          // 自动平仓时间-小时（北京时间）
input int    AutoCloseMinute      = 55;         // 自动平仓时间-分钟（北京时间）
input string NoTradeTime1Start    = "08:55";    // 禁止交易时段1开始（北京时间）
input string NoTradeTime1End      = "09:05";    // 禁止交易时段1结束（北京时间）
input string NoTradeTime2Start    = "22:25";    // 禁止交易时段2开始（北京时间）
input string NoTradeTime2End      = "22:35";    // 禁止交易时段2结束（北京时间）
...
int      g_reversalAtrHandle     = INVALID_HANDLE;
...
//--- 时间控制状态
int      g_lastAutoCloseDayKey  = -1;    // 上次自动平仓的北京时间日期，防止重复平仓
...
//| 检查结构性反转过滤所需K线是否足够                                |
//+------------------------------------------------------------------+
bool HasEnoughReversalBars()
{
   int requiredBars = ReversalLookbackBars + MathMax(ReversalStructureBreakBars, 2) + 3;
   return (EnableStructuralReversalFilter &&
           ReversalLookbackBars > 1 &&
           ReversalAtrPeriod > 0 &&
           iBars(_Symbol, PERIOD_M5) > requiredBars);
}

//+------------------------------------------------------------------+
//| 获取已收盘K线数据                                                  |
//+------------------------------------------------------------------+
bool GetClosedBarData(int shift, double &openPrice, double &highPrice, double &lowPrice, double &closePrice)
{
   openPrice = iOpen(_Symbol, PERIOD_M5, shift);
   highPrice = iHigh(_Symbol, PERIOD_M5, shift);
   lowPrice = iLow(_Symbol, PERIOD_M5, shift);
   closePrice = iClose(_Symbol, PERIOD_M5, shift);

   return (openPrice > 0 && highPrice > 0 && lowPrice > 0 && closePrice > 0 && highPrice >= lowPrice);
}

//+------------------------------------------------------------------+
//| 获取反转过滤ATR                                                    |
//+------------------------------------------------------------------+
double GetReversalAtrValue()
{
   if(g_reversalAtrHandle == INVALID_HANDLE)
      return 0.0;

   double atrValue[];
   ArraySetAsSeries(atrValue, true);

   if(CopyBuffer(g_reversalAtrHandle, 0, 1, 1, atrValue) < 1)
      return 0.0;
...
//| 计算结构危险区窗口统计                                             |
//+------------------------------------------------------------------+
bool GetReversalWindowStats(double currentPrice,
                            double &move,
                            double &windowHigh,
                            double &windowLow,
...
{
   if(!HasEnoughReversalBars())
      return false;

   double latestClosed = iClose(_Symbol, PERIOD_M5, 1);
   double startClosed = iClose(_Symbol, PERIOD_M5, ReversalLookbackBars + 1);

   if(latestClosed <= 0 || startClosed <= 0)
      return false;

   windowHigh = -DBL_MAX;
   windowLow = DBL_MAX;

   for(int i = 1; i <= ReversalLookbackBars; i++)
   {
      double high = iHigh(_Symbol, PERIOD_M5, i);
      double low = iLow(_Symbol, PERIOD_M5, i);
   ...
   atrValue = GetReversalAtrValue();
   if(atrValue <= 0)
      return false;

   move = latestClosed - startClosed;
   rangePosition = (currentPrice - windowLow) / range;
   sprintThreshold = atrValue * ReversalSprintAtrMultiplier;

   return (sprintThreshold > 0);
}

//+------------------------------------------------------------------+
//| 判断长上影线                                                       |
//+------------------------------------------------------------------+
bool HasLongUpperShadow(string &reason)
{
   double openPrice = 0, highPrice = 0, lowPrice = 0, closePrice = 0;
   if(!GetClosedBarData(1, openPrice, highPrice, lowPrice, closePrice))
      return false;

   double body = MathMax(MathAbs(closePrice - openPrice), _Point);
   double upperShadow = highPrice - MathMax(openPrice, closePrice);
   double lowerShadow = MathMin(openPrice, closePrice) - lowPrice;

   if(upperShadow >= body * ReversalShadowBodyRatio &&
      (lowerShadow <= 0 || upperShadow >= lowerShadow * ReversalShadowOppositeRatio))
   {
      reason = "长上影";
      return true;
   }
...
//| 判断长下影线                                                       |
//+------------------------------------------------------------------+
bool HasLongLowerShadow(string &reason)
{
   double openPrice = 0, highPrice = 0, lowPrice = 0, closePrice = 0;
   if(!GetClosedBarData(1, openPrice, highPrice, lowPrice, closePrice))
      return false;
   ...
   if(lowerShadow >= body * ReversalShadowBodyRatio &&
      (upperShadow <= 0 || lowerShadow >= upperShadow * ReversalShadowOppositeRatio))
   {
      reason = "长下影";
      return true;
   }
...
//| 判断看跌吞没/压入式反转                                            |
//+------------------------------------------------------------------+
bool HasBearishEngulfingOrRejection(string &reason)
{
   if(!EnableReversalEngulfingCheck)
      return false;

   double open1 = 0, high1 = 0, low1 = 0, close1 = 0;
   double open2 = 0, high2 = 0, low2 = 0, close2 = 0;

   if(!GetClosedBarData(1, open1, high1, low1, close1) ||
      !GetClosedBarData(2, open2, high2, low2, close2))
      return false;

   if(close2 <= open2 || close1 >= open1)
      return false;

   double prevBody = close2 - open2;
   if(prevBody <= 0)
      return false;
   ...
   if(close1 <= close2 - prevBody * 0.5 && high1 >= high2 - ReversalBreakoutBufferUsd)
   {
      reason = "乌云压入";
      return true;
   }
...
//| 判断看涨吞没/反抽式反转                                            |
//+------------------------------------------------------------------+
bool HasBullishEngulfingOrRejection(string &reason)
{
   if(!EnableReversalEngulfingCheck)
      return false;
   ...
   if(close1 >= close2 + prevBody * 0.5 && low1 <= low2 + ReversalBreakoutBufferUsd)
   {
      reason = "看涨反抽";
      return true;
   }
...
//| 判断短线结构跌破                                                   |
//+------------------------------------------------------------------+
bool HasShortTermLowBreak(string &reason)
{
   if(ReversalStructureBreakBars <= 0)
      return false;
   ...
   for(int i = 2; i <= ReversalStructureBreakBars + 1; i++)
   {
      double low = iLow(_Symbol, PERIOD_M5, i);
      if(low <= 0)
         return false;
   ...
   if(close1 < recentLow - ReversalBreakoutBufferUsd)
   {
      reason = "短线低点跌破";
      return true;
   }
...
//| 判断短线结构突破                                                   |
//+------------------------------------------------------------------+
bool HasShortTermHighBreak(string &reason)
{
   if(ReversalStructureBreakBars <= 0)
      return false;
   ...
   if(close1 > recentHigh + ReversalBreakoutBufferUsd)
   {
      reason = "短线高点突破";
      return true;
   }
...
//| 判断看跌反转证据                                                   |
//+------------------------------------------------------------------+
bool HasBearishReversalEvidence(string &reason)
{
   if(HasLongUpperShadow(reason))
      return true;
   if(HasBearishEngulfingOrRejection(reason))
      return true;
   if(HasShortTermLowBreak(reason))
      return true;
...
//| 判断看涨反转证据                                                   |
//+------------------------------------------------------------------+
bool HasBullishReversalEvidence(string &reason)
{
   if(HasLongLowerShadow(reason))
      return true;
   if(HasBullishEngulfingOrRejection(reason))
      return true;
   if(HasShortTermHighBreak(reason))
      return true;
...
//| 判断是否突破危险区并恢复顺势交易                                   |
//+------------------------------------------------------------------+
bool IsBreakoutRelease(int trend, double currentPrice, double windowHigh, double windowLow)
{
   double close1 = iClose(_Symbol, PERIOD_M5, 1);

   if(trend == 1)
   {
      double breakoutBoundary = -DBL_MAX;
      for(int i = 2; i <= ReversalLookbackBars + 1; i++)
      {
         double high = iHigh(_Symbol, PERIOD_M5, i);
         if(high <= 0)
            return false;
         if(high > breakoutBoundary)
            breakoutBoundary = high;
      }

      double breakoutLevel = breakoutBoundary + ReversalBreakoutBufferUsd;
      bool tickBreakout = (currentPrice > breakoutLevel);
      bool closedBarBreakout = (close1 > breakoutLevel);

      if(ReversalBreakoutConfirmMode == BREAKOUT_BY_TICK)
         return tickBreakout;
      if(ReversalBreakoutConfirmMode == BREAKOUT_BY_CLOSED_BAR)
         return closedBarBreakout;
      return (tickBreakout || closedBarBreakout);
   }
   ...
   if(trend == -1)
   {
      ...
      for(int i = 2; i <= ReversalLookbackBars + 1; i++)
      {
         ...
         if(low <= 0)
            return false;
         if(low < breakoutBoundary)
            breakoutBoundary = low;
      }

      double breakoutLevel = breakoutBoundary - ReversalBreakoutBufferUsd;
      bool tickBreakout = (currentPrice < breakoutLevel);
      bool closedBarBreakout = (close1 > 0 && close1 < breakoutLevel);
...
//+------------------------------------------------------------------+
//| 结构性反转危险区过滤                                               |
//| 返回: true=禁止当前方向开仓, false=允许继续原有逻辑                |
//+------------------------------------------------------------------+
bool ShouldBlockStructuralReversalTrade(int trend)
{
   if(!EnableStructuralReversalFilter)
      return false;
   ...
   double currentPrice = (trend == 1 ? tick.ask : tick.bid);
   double move = 0.0, windowHigh = 0.0, windowLow = 0.0, rangePosition = 0.0;
   double atrValue = 0.0, sprintThreshold = 0.0;

   if(!GetReversalWindowStats(currentPrice, move, windowHigh, windowLow, rangePosition, atrValue, sprintThreshold))
   {
      LogDebug("结构反转过滤：数据不足或ATR无效，沿用原交易逻辑");
      return false;
   }

   if(IsBreakoutRelease(trend, currentPrice, windowHigh, windowLow))
   {
      LogDebug("结构反转过滤：危险区已突破，恢复顺势交易。方向=" + (trend == 1 ? "多" : "空") +
               " 当前价=" + DoubleToString(currentPrice, _Digits) +
               " 高点=" + DoubleToString(windowHigh, _Digits) +
               " 低点=" + DoubleToString(windowLow, _Digits));
      return false;
   }
   ...
   {
      string reason = "";
      bool upwardSprint = (move >= sprintThreshold);
      bool nearHighZone = (rangePosition >= ReversalHighZoneRatio);
      bool bearishEvidence = HasBearishReversalEvidence(reason);

      if(upwardSprint && nearHighZone)
      {
         LogDebug("结构反转观察区：上涨末端高位。涨幅=" + DoubleToString(move, 2) +
                  " 阈值=" + DoubleToString(sprintThreshold, 2) +
                  " 区间位置=" + DoubleToString(rangePosition * 100, 1) + "%" +
                  " 高点=" + DoubleToString(windowHigh, _Digits) +
                  " 当前价=" + DoubleToString(currentPrice, _Digits) +
                  " 反转证据=" + (bearishEvidence ? reason : "暂无"));
      }

      if(upwardSprint && nearHighZone && bearishEvidence)
      {
         LogInfo("到达结构反转点：禁止追多。原因=" + reason +
                 " 涨幅=" + DoubleToString(move, 2) +
                 " 阈值=" + DoubleToString(sprintThreshold, 2) +
                 " 区间位置=" + DoubleToString(rangePosition * 100, 1) + "%" +
                 " 高点=" + DoubleToString(windowHigh, _Digits));
         return true;
   ...
   {
      string reason = "";
      bool downwardSprint = (move <= -sprintThreshold);
      bool nearLowZone = (rangePosition <= ReversalLowZoneRatio);
      bool bullishEvidence = HasBullishReversalEvidence(reason);

      if(downwardSprint && nearLowZone)
      {
         LogDebug("结构反转观察区：下跌末端低位。跌幅=" + DoubleToString(MathAbs(move), 2) +
                  " 阈值=" + DoubleToString(sprintThreshold, 2) +
                  " 区间位置=" + DoubleToString(rangePosition * 100, 1) + "%" +
                  " 低点=" + DoubleToString(windowLow, _Digits) +
                  " 当前价=" + DoubleToString(currentPrice, _Digits) +
                  " 反转证据=" + (bullishEvidence ? reason : "暂无"));
      }

      if(downwardSprint && nearLowZone && bullishEvidence)
      {
         LogInfo("到达结构反转点：禁止追空。原因=" + reason +
                 " 跌幅=" + DoubleToString(MathAbs(move), 2) +
                 " 阈值=" + DoubleToString(sprintThreshold, 2) +
                 " 区间位置=" + DoubleToString(rangePosition * 100, 1) + "%" +
                 " 低点=" + DoubleToString(windowLow, _Digits));
         return true;
...
//+------------------------------------------------------------------+
//| 检查当前北京时间是否在禁止交易时段内                               |
//| 返回: true=在禁止交易时段, false=可以交易                         |
//+------------------------------------------------------------------+
bool IsInNoTradeTime()
{
   ...
   // 检查禁止交易时段1
   int start1Hour = 0, start1Minute = 0, end1Hour = 0, end1Minute = 0;
   ParseTimeString(NoTradeTime1Start, start1Hour, start1Minute);
   ParseTimeString(NoTradeTime1End, end1Hour, end1Minute);
   int startTime1 = start1Hour * 60 + start1Minute;
   int endTime1 = end1Hour * 60 + end1Minute;

   if(currentTimeValue >= startTime1 && currentTimeValue < endTime1)
   {
      LogDebug("当前在禁止交易时段1: " + NoTradeTime1Start + " - " + NoTradeTime1End);
      return true;
   }

   // 检查禁止交易时段2
   int start2Hour = 0, start2Minute = 0, end2Hour = 0, end2Minute = 0;
   ParseTimeString(NoTradeTime2Start, start2Hour, start2Minute);
   ParseTimeString(NoTradeTime2End, end2Hour, end2Minute);
   int startTime2 = start2Hour * 60 + start2Minute;
   int endTime2 = end2Hour * 60 + end2Minute;

   if(currentTimeValue >= startTime2 && currentTimeValue < endTime2)
   {
      LogDebug("当前在禁止交易时段2: " + NoTradeTime2Start + " - " + NoTradeTime2End);
      return true;
   }
...
//+------------------------------------------------------------------+
//| 检查是否到达自动平仓时间并执行平仓                                 |
//| 返回: true=已执行平仓或无需平仓, false=平仓失败                   |
//+------------------------------------------------------------------+
bool CheckAndExecuteAutoClose()
{
   if(!EnableTimeControl)
      return true;
   ...
   // 如果今天已经执行过自动平仓，不再重复
   if(g_lastAutoCloseDayKey == currentDayKey)
      return true;

   int currentHour = 0, currentMinute = 0;
   GetBeijingHourMinute(currentHour, currentMinute);

   // 检查是否到达自动平仓时间（精确匹配到分钟）
   if(currentHour == AutoCloseHour && currentMinute == AutoCloseMinute)
   {
      // 检查是否有持仓
      double openLots = GetOpenLots();
      if(openLots <= 0)
      {
         g_lastAutoCloseDayKey = currentDayKey;
         return true;
      }

      LogInfo("★★★ 到达自动平仓时间 " + IntegerToString(AutoCloseHour) + ":" + IntegerToString(AutoCloseMinute) +
              "，开始平仓所有持仓...");
      ...
      for(int i = PositionsTotal() - 1; i >= 0; i--)
      {
         ...
         {
            closedCount++;
            LogInfo("自动平仓成功 Ticket=" + IntegerToString((int)ticket));
         }
         else
         {
            failedCount++;
            LogInfo("自动平仓失败 Ticket=" + IntegerToString((int)ticket) +
      ...
      {
         g_lastAutoCloseDayKey = currentDayKey;
         LogInfo("自动平仓完成。成功=" + IntegerToString(closedCount) + " 失败=" + IntegerToString(failedCount));
         return true;
      }
...
int OnInit()
{
   ...
   g_reversalAtrHandle = iATR(_Symbol, PERIOD_M5, ReversalAtrPeriod);
   if(g_reversalAtrHandle == INVALID_HANDLE)
   {
      LogInfo("创建结构反转ATR指标失败");
      return INIT_FAILED;
   }
   ...
   {
      LogInfo("时间控制已启用");
      LogInfo("自动平仓时间=" + IntegerToString(AutoCloseHour) + ":" + IntegerToString(AutoCloseMinute) + " (北京时间)");
      LogInfo("禁止交易时段1=" + NoTradeTime1Start + " - " + NoTradeTime1End);
      LogInfo("禁止交易时段2=" + NoTradeTime2Start + " - " + NoTradeTime2End);
   }
   else
   {
      LogInfo("时间控制已禁用");
   ...
   if(EnableStructuralReversalFilter)
   {
      LogInfo("结构反转过滤已启用 回看K线=" + IntegerToString(ReversalLookbackBars) +
              " ATR倍数=" + DoubleToString(ReversalSprintAtrMultiplier, 2) +
              " 高位区=" + DoubleToString(ReversalHighZoneRatio * 100, 0) + "%" +
              " 低位区=" + DoubleToString(ReversalLowZoneRatio * 100, 0) + "%");
   }
   else
   {
      LogInfo("结构反转过滤已禁用");
...
void OnDeinit(const int reason)
{
   ...
   if(g_reversalAtrHandle != INVALID_HANDLE)
      IndicatorRelease(g_reversalAtrHandle);
...
void OnTick()
{
   ...
   if(currentDayKey != g_lastBeijingDayKey)
   {
      ...
      // 跨日时重置自动平仓标记，允许新的一天再次执行
      g_lastAutoCloseDayKey = -1;
   }

   // 检查并执行自动平仓（每天8:55北京时间）
   if(!CheckAndExecuteAutoClose())
   {
      LogDebug("自动平仓执行失败，将在下一个tick重试");
      return;
   }
   ...
   {
      // 多头趋势，做多
      if(ShouldBlockStructuralReversalTrade(trend))
         return;
